...
public class EGA_Laser : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            if (Physics.SphereCast(transform.position, 0.2f, transform.TransformDirection(Vector3.forward), out hit, MaxLength, _layerMask, QueryTriggerInteraction.Ignore))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {

                if (hit.collider.TryGetComponent(out PhysicHero _physicHero))
                {
                    _physicHero.PublicHeroDieAction.TakeDamage(5, hit.transform.position);
                }
