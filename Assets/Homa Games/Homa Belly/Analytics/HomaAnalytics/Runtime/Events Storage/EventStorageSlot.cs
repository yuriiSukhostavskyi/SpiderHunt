...
namespace HomaGames.HomaBelly
{
    public class EventStorageSlot
    {
        ...
        public async Task CloseSlot()
        {
            ...
            await TryToDisposeStreamWhenClosed();
        }
        ...
        private async void ReadEventsAndInitializeSlot()
        {
            lock (m_fileStreamLock)
            {
            ...
            try
            {
                ...
                while (m_stream.Position < m_stream.Length)
                {
                    ...
                    {
                        Debug.LogError($"[ERROR] Error reading event name: {this}");
                        ClearStream();
                        return;
                    }
                    ...
                    {
                        Debug.LogError($"[ERROR] Error reading event id: {this}");
                        ClearStream();
                        return;
                    }
                    ...
                    {
                        Debug.LogError($"[ERROR] Error reading event body: {this}");
                        ClearStream();
                        return;
                    }
                    ...
                    var totalLength = eventName.Length + eventId.Length + eventBody.Length;
        ...
        /// </summary>
        private async void WritePendingEventsIfIdle()
        {
            lock (m_stateLock)
            {
        ...
        private async Task WriteBytesInStream(List<PendingEvent> eventsBatchToSave)
        {
            ...
            for (int i = 0; i < eventsBatchToSave.Count; i++)
            {
                ...
                // Event name
                var eventNameBytes = Encoding.UTF8.GetBytes(pendingEvent.EventName);
                allEventBytes.AddRange(BitConverter.GetBytes(eventNameBytes.Length));
                allEventBytes.AddRange(eventNameBytes);

                // Event id
                var eventIdBytes = Encoding.UTF8.GetBytes(pendingEvent.Id);
                allEventBytes.AddRange(BitConverter.GetBytes(eventIdBytes.Length));
                allEventBytes.AddRange(eventIdBytes);

                // Event body
                var eventBodyBytes = Encoding.UTF8.GetBytes(pendingEvent.Json);
                allEventBytes.AddRange(BitConverter.GetBytes(eventBodyBytes.Length));
                allEventBytes.AddRange(eventBodyBytes);

                // Write total length of all fields at the end of the event. We will use this
                // to validate data when reading
                var totalLength = eventNameBytes.Length + eventIdBytes.Length + eventBodyBytes.Length;
                SizeInBytes += totalLength;
        ...
        private async Task<string> ReadFieldFromFile(int maxFieldSize)
        {
            ...
            if (eventFieldSize <= 0 || eventFieldSize > maxFieldSize)
            {
        ...
        private async void ChangeState(EventStorageSlotStates newState)
        {
            lock (m_stateLock)
            {
                if (newState == State)
                {
                    Debug.LogError($"[ERROR] You can't change to the same state: {newState} {SlotName}");
                    return;
                }
            ...
            if (State == EventStorageSlotStates.Idle)
            {
                if (IsClosed() && !m_disposed)
                {
                    // Event closed while it was reading or writing
                    await TryToDisposeStreamWhenClosed();
                }
                else if (HasPendingEventsToWrite())
                {
        ...
        private void AddSavedEvent(params PendingEvent[] pendingEvents)
        {
            lock (m_savedEventsLock)
            {
                ...
                // After adding an event, start the auto dispatch timer
        ...
        {
            lock(m_savedEventsLock) lock (m_eventsToSaveLock) 
            {
        ...
        private async void DelayedDispatchSlot(int milliseconds)
        {
